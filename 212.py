...
def image(image):
    ...
    cv.imshow("", live_feed_from_camera)
    cv.waitKey(1)
    return
...
def camera(get_blueprint_of_world):
    ...
    camera_sensor.set_attribute('image_size_x',f='{IM_HEIGHT}')
    camera_sensor.set_attribute('image_size_y',f='{IM_WIDTH}')
...
try:
    ...
    dropped_vehicle.set(autopilot=True)
